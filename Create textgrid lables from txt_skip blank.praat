...
## This script is based on Mietta Lennes' script and added the functions
## 1. if choose NOT to overwrite, an additional tier is added to the top;
## 2. intervals without any labels in the orignal tier are skiped instead of 'xxx'
...
sentence Filename C:\Users\rand4.txt
...
comment Do you want to overwrite old labels? The original tier will be replaced.
comment (If you choose to NOT overwrite, a new tier will be added to the top.)
boolean Overwrite 0
endform
...
oldlabel$ = Get label of interval... tier interval
	...
	# we add the new label we collected from the text file:
		if overwrite = 1
			...
			Set interval text... 1 interval 'newlabel$'
	   endif
	endif
endfor
else 
